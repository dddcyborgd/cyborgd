...
| `event` | `{"type":"event","name":"gesture","data":{"name":"smile"}}` | names: `portal{to}` `riddle{agent}` `reach{zone}` `gesture{name}` `focus{agent|null}` |
...
| `mirror` | `event{gesture}` from a player within 6 m (`smile` `jawOpen` `browsUp` `nod`) | faces the player | mirrors the gesture as its animation + a line (`jawOpen` → emotion `curious`) |
