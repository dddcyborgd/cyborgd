...
| `event` | `{"type":"event","name":"gesture","data":{"name":"smile"}}` | names: `portal{to}` `riddle{agent}` `reach{zone}` `gesture{name}` `focus{agent|null}` `item{name,action}` (action: use · raise · lower · select — the participant's sphere of influence: sceptre, orb…) |
