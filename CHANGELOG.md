...
- The hierarchy's two dials: `registries/field-policy.json` (per-rung `outflow`/`inflow`), `GET /field-policy`, `POST /field-policy` (OVERSEER+ claim), `welcome.policy` + `welcome.fieldMax`; field modes `open` · `connected` (links; delivered only to links) · `private` (signer only; presence-only participants fall back to open); `core/field.mjs` (pure) + test/field.test.mjs.
